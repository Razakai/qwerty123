...
class Database:
    ...
    def createTables(self):
        ...
        cursor.execute(
            "create table metrics (id integer PRIMARY KEY AUTOINCREMENT, sensor_id varchar2(255) not null, "
            "temperature float not null, humidity float not null, timestamp int not null, FOREIGN KEY (sensor_id) "
    ...
    def fetchDB(self, query, isOne, values):
        ...
        try:
            if isOne:
                return cursor.execute(query, values).fetchone() if values is not None else cursor.execute(
                    query).fetchone()
